...
def create_augmented_dataloader(args, dataset):
    ...
    sampled_train = original_train.shuffle(seed=42).select(range(5000))
    
    # Apply transformation to the sampled examples
    transformed_train = sampled_train.map(custom_transform, load_from_cache_file=False)
    
    # Concatenate original training data with transformed examples
    augmented_dataset = datasets.concatenate_datasets([original_train, transformed_train])
...
if __name__ == "__main__":
    ...
    # Workaround because having issues loading the dataset:
    # Load train and test separately to avoid unsupervised split verification issue
    try:
        dataset = load_dataset("imdb")
    except Exception as e:
        if "unsupervised" in str(e):
            # Load splits separately with ignore_verifications and combine
            train_ds = load_dataset("imdb", split="train", ignore_verifications=True)
            test_ds = load_dataset("imdb", split="test", ignore_verifications=True)
            # Create DatasetDict manually
